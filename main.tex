...
\begin{document}
\homework{Programming Assignment \uppercase\expandafter{\romannumeral1}}{Due: 09/23/2022 at 11:59PM EST}{Professor H.T. Kung}{}{Name-1, Name-2, Name-3, Name-4}{}{}
...
& \;\;\;\;\;\;\;\;\;\;\;\;  + (Nr)_{B_{read}[1:N][qp+1:N]} + \sum_{i=(l-1)p+1}^{lp}\sum_{j=qp+1}^N(1_{C_{read}[i][j]}+1_{C_{write}[i][j]}) \bigg] \\
 ...
 \\
& \;\;\;\;\;\;\;\; +  (Nr)_{B_{read}[1:N][qp+1:N]} + \sum_{i=qp+1}^N \sum_{j=qp+1}^N (1_{C_{read}[i][j]} + 1_{C_{write}[i][j]}) \\
& = [qpN+q^2Np+q^22p^2+qNr+q2pr]+ [rN+qNp+q2rp] + [Nr+2r^2]  \\
& = 2Nqp + 2Nr + 2qpr + 2r^2 + Nq^2p + Nqr + 2q^2p^2 + 2qpr\\
& = 2N(qp+r) + 2r(qp+r) + Nq(qp+r) + 2qp(qp+r)\\
& = 2N^2+2Nr+N^2q+2Nqp\\
& = 2N^2 + 2N(qp+r) + N^2q\\
& = 4N^2 + N^2\bigg\lfloor\frac{N}{p}\bigg\rfloor
\end{align*}

Note: by default $r>0$ and so this equation is not valid when $r=0$; in order to make it valid when $r=0$ must subtract $N^2$ whenever $r=0$.

$$ \alpha = \frac{2N^3}{IO} = \frac{2N^3}{4N^2 + N^2\bigg\lfloor\frac{N}{p}\bigg\rfloor} = \frac{2N}{4+\bigg\lfloor\frac{N}{p}\bigg\rfloor}$$
...
This can be seen by setting $p=\frac{N+2}{8}$ and seeing that $\alpha$ for 3.2 becomes $\alpha=\frac{N}{4+\big\lfloor\frac{8N}{N+2}\big\rfloor}$}. Note that  $\frac{8N}{N+2}$ is larger than 2 for any valid values of N (6, 14, 22,...) and so see that with the same memory the inner product doesn't have as high of an arithmetic intensity. This would be clearer if I had also derived the general expression for the outer-product in terms of a block size $p$. 
